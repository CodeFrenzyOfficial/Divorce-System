...
import { states } from "../data/stateNames";
...
export default function Footer() {
  return (
    <div className="px-10">
      ...
      <div className="grid grid-cols-6 gap-2 my-4 mx-auto">
        {Array.from({ length: 6 }).map((_, colIndex) => (
          <div key={colIndex} className="flex flex-col">
            {states.slice(colIndex * 10, (colIndex + 1) * 10).map((state) => (
              <StateLink key={state} state={state} />
            ))}
          </div>
        ))}
      </div>
